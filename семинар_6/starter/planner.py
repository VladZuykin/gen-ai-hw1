...
ПРАВИЛА:
1. Если итоговый ответ требует арифметики (разность, отношение, процент, произведение),
    ты ОБЯЗАН добавить отдельный подвопрос с expected_tools=["calculate"].
    НЕЛЬЗЯ считать в уме и выдавать результат без calculate.
